...
`Expr` 定义在 `Lean/Expr.lean` 中，有以下核心构造子：
...
# 2.4 模式匹配 Expr：`match_expr` 和 `let_expr`
%%%
...
> *注意*：`match_expr` 和 `let_expr` 是专门的模式匹配语法糖，不是普通函数名。你不能对它们用 `#check`（不像 `forallTelescope` 那样是一个可查类型的函数）。它们的作用是按 *head constant* 匹配表达式（如 `Eq`、`And`、`Or`），自动拆开嵌套的 `app`。对于 `forallE` 这类 binder 构造子，应该用普通的 `Expr.isForall` 或直接 `match`。
...
读取 `Expr` 用模式匹配，构造 `Expr` 用 `mkApp` 系列函数：
...
这更方便，但有额外开销（要走一遍 elaboration）。在性能敏感的 tactic 中，手动 `mkApp` 更好。

# 2.6 de Bruijn 索引 vs 自由变量
%%%
...
在元程序中，我们通常借助 `forallTelescope`、`lambdaTelescope` 等 API 打开绑定结构；这些 API 会把 body 中对应的 `bvar` 替换成当前局部上下文里的 `fvar`，因此日常 tactic 代码里更常直接操作 `fvar`。关键 API：
...
`forallTelescope` 和 `lambdaTelescope` 是处理绑定结构的标准方式。它们：
1. 创建新的 `fvar` 加入局部上下文
...
`match_expr` 按 head constant 匹配，但 `whnf` 没有自动调用。如果目标被 `let` 或 `abbrev` 包了一层，需要先 `whnf`：
...
`forallTelescope` 里创建的 `fvar` 只在回调内有效。如果你把它存到外面的变量里再使用，会得到 `unknown free variable` 错误。
...
`mkApp3 (mkConst Eq) a b` 会构造出类型错误的表达式（`Eq` 需要 3 个参数：类型 α、lhs、rhs）。这类错误不会立即 panic，而是在后续 `isDefEq` 或 `check` 时才报 type mismatch。
