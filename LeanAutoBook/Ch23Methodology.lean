...
- 层级：*macro* —— 适用场景：tactic 序列的固定组合 —— 实现成本：极低（1-10 行） —— 灵活性：低
- 层级：*simp/aesop 扩展* —— 适用场景：可用引理集覆盖的模式 —— 实现成本：低（注册引理） —— 灵活性：中
- 层级：*elab + MetaM* —— 适用场景：需要检查目标结构或搜索假设 —— 实现成本：中（20-100 行） —— 灵活性：高
- 层级：*反射*（进阶） —— 适用场景：可判定问题，需要高性能 —— 实现成本：高（200+ 行） —— 灵活性：域特定
- 层级：*外部工具*（进阶） —— 适用场景：超出 Lean 计算能力 —— 实现成本：高 —— 灵活性：域特定
