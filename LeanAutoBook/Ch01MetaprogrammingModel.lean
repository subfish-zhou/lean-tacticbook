...
- *{moduleTerm}`@MetavarContext`*：所有元变量（{moduleTerm}`@MVarId`）的声明和赋值
...
- {moduleTerm}`@MetaM`：核心状态为 {moduleTerm}`@LocalContext`、{moduleTerm}`@MetavarContext`，典型操作为类型检查、创建元变量
...
%%%
tag := "failure-unknown-identifier"
%%%
