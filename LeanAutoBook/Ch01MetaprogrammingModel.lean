...
- `MVarId.assign` 才是*真正把证明填进去*的那一步——它给目标对应的证明元变量赋值，等于往那张带洞的证明里合法地填结构。玩具里“把洞补死”对应的就是它。
...
- 队列和“未填洞集合”之间由 `pruneSolvedGoals`/`getUnsolvedGoals` 这类操作*对账*：把那些其实已经被 `assign` 掉的元变量从队列里剔掉，让工作队列重新和真实的未解决集合对齐。

一句话：*`assign` 负责证明，`setGoals` 只负责待办清单，别拿后者当前者*。你已经知道每一步跑在哪一层、能读什么、能改什么、失败会怎样冒泡。上一章那个 `my_assumption` 的伪代码，你现在有能力把它翻成真的 Lean。
