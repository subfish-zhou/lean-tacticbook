...
- `MVarId.assign` 是*真正把表达式写进证明洞*的低层步骤——它给目标对应的证明元变量赋值。这里不能把“写进去”和“已经合法”混为一谈：`assign` 本身不是独立的类型检查器，规范的 tactic 应先构造并检查类型正确的证明表达式，再赋值；重复赋值、循环赋值或类型不对的表达式仍可能在后续检查中被拒绝。玩具里“把洞补死”对应的是“检查后赋值”这一整套动作，不是裸调用一个写操作。
