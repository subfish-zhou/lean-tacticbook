...
*本章目标*：从零实现几个有实际用途的 tactic，掌握 {lean}`TacticM` 的核心模式。
...
第二章中，你学习了 Lean 4 内部表示表达式的核心数据类型 {lean}`Expr`。你已经知道 `Expr.app`、`Expr.const`、`Expr.mvar` 等构造子分别代表什么，也知道如何用 {lean}`mkApp`、{lean}`mkConst` 等辅助函数构造表达式。本章要做的事情是：*把 Expr 的知识用起来*。

Tactic 的本质是什么？它是一个接收当前证明状态、返回新证明状态的函数。而"证明状态"的核心就是一组尚未填充的元变量（metavariable），每个元变量对应一个待证目标。Tactic 的工作就是读取这些元变量的类型（即目标命题），构造出合适的 {lean}`Expr` 来填充它们，或者把一个目标拆分成更小的子目标。
...
1. *接收语法*：Lean 解析器根据你用 `syntax` 或 `elab` 注册的语法规则进行匹配，把解析出的 {lean}`Syntax` 节点传给你的函数。如果你的 tactic 接受参数（比如一个假设名），参数也在这个 {lean}`Syntax` 里。

2. *读取状态*：从 {lean}`TacticM` monad 中获取当前目标列表。最常用的是 {lean}`getMainGoal`（取第一个目标）和 {lean}`getGoals`（取全部目标）。每个目标是一个 {lean}`MVarId`——元变量的唯一标识符。

3. *分析目标*：通过 `goal.getType` 拿到目标的类型（一个 {lean}`Expr`），然后用第二章学到的模式匹配技术分析它的结构。比如判断它是否形如 `P ∧ Q`，或者是否是一个等式 `a = b`。

4. *构造证明项并更新状态*：这是最关键的一步。你需要构造一个 {lean}`Expr` 来"回答"当前目标。如果证明项中还有未填充的部分（hole），那些 hole 就变成新的子目标。最后调用 `goal.assign proof` 关闭当前目标，调用 {lean}`setGoals` 设置新的目标列表。
...
*`let target ← goal.getType`*：获取目标的类型，即待证命题。返回值是一个 {lean}`Expr`。

*`let lctx ← getLCtx`*：获取当前局部上下文，它是一个 {lean}`LocalContext`，包含所有局部声明（local declaration）。

*`for decl in lctx do`*：遍历所有局部声明。每个 `decl` 是一个 {lean}`LocalDecl`，包含 `.userName`（用户可见的名字）、`.type`（类型）、`.toExpr`（对应的自由变量表达式，即 `Expr.fvar`）等字段。
...
*`let_expr And P Q := target | ...`*：这是 Lean 提供的宏，专门用于对 {lean}`Expr` 做模式匹配。它会检查 `target` 是否形如 `@And P Q`（即 `Expr.app (Expr.app (Expr.const And _) P) Q`），如果是则绑定 `P` 和 `Q`，否则执行 `|` 后面的分支。这比手写 `match` 简洁很多。

*`let mvarP ← mkFreshExprMVar P`*：创建一个类型为 `P` 的新元变量。{lean}`mkFreshExprMVar` 返回 {lean}`Expr`（具体来说是一个 `Expr.mvar`），这个表达式代表一个尚未填充的 hole。`.mvarId!` 可以取出它的 {lean}`MVarId`。

*`let proof := mkApp4 (mkConst And.intro) P Q mvarP mvarQ`*：构造表达式 `@And.intro P Q ?mvarP ?mvarQ`。{lean}`mkApp4` 是 {lean}`mkApp` 的四参数版本。注意 `And.intro` 的完整签名是 `And.intro : {a b : Prop} → a → b → a ∧ b`，这里四个参数分别是 `a := P`、`b := Q`、以及两个证明。
...
*`setGoals (newGoals ++ others)`*：把两个新子目标放在前面，其余未处理的目标跟在后面。*这一步不能遗漏*——如果你不调用 {lean}`setGoals`，新创建的元变量不会出现在目标列表中，用户在 Infoview 里看不到它们，也无法用后续 tactic 处理它们。

这个例子展示了 tactic 最核心的模式：*分析目标结构 → 构造部分证明项（带 hole）→ 把 hole 变成新的子目标*。几乎所有非平凡的 tactic 都遵循这个模式。

> *{lean}`replaceMainGoal` vs {lean}`setGoals` 的选择*
>
> - *只替换当前主目标*时，用 `replaceMainGoal newGoals`——它自动把主目标替换为 `newGoals`，并保留其余目标，不需要手动做 `erase`。
> - *需要完全控制整个目标列表*时（比如重新排序、批量过滤），才用 {lean}`setGoals`。
>
> 经验法则：如果你的 tactic 只处理一个目标并产出若干子目标，{lean}`replaceMainGoal` 更简洁也更安全。
...
手动构造 {lean}`Expr` 虽然灵活，但很多时候你不需要从头来——Lean 标准库已经提供了大量 tactic，你可以在自己的 tactic 中直接调用它们。这就是"组合式"tactic 的思路：把已有的自动化当作积木块来组合。
...
核心 API 是 *{lean}`evalTactic`*：它接收一个 {lean}`Syntax` 节点，执行对应的 tactic。`` ← `(tactic| ...) `` 是 Lean 的 quotation 语法，用于在代码中构造 {lean}`Syntax`。

这段代码的策略是*搜索 + 回退*：先试最简单的方法（直接在假设中找精确匹配），失败了再试 {lean}`rfl`（关闭 `a = a` 形式的等式目标）。`try ... catch _ => pure ()` 确保第二个策略失败时不会中断整个 tactic，而是回退到最后的 `throwTacticEx`。

你也可以用 {lean}`evalTactic` 一次性表达同样的逻辑：
...
`first` 会依次尝试列出的 tactic，直到有一个成功。这种写法更简洁，但手动写搜索循环给你更多控制权——比如你可以过滤掉特定类型的假设，或者按照特定顺序搜索。直接操作 {lean}`Expr` 也比通过 {lean}`evalTactic` 调用更高效，因为省去了语法解析和 tactic 调度的开销。在性能敏感的场景（比如要在大量目标上运行的自动化），这种混合风格很常见。
...
前面的 tactic 都只处理第一个目标（{lean}`getMainGoal`）。但有时你希望对所有未解决的目标执行同一操作——比如"把所有能用 {kw}`assumption` 关掉的目标都关掉"。
...
*`if ← g.isAssigned then continue`*：检查目标是否已经被赋值。为什么需要这个检查？因为 {lean}`isDefEq` 有副作用——它在判断等价性的过程中可能会实例化（instantiate）某些元变量。这意味着在处理目标 A 时调用 {lean}`isDefEq`，可能会"顺带"解决了目标 B。所以在处理每个目标之前，先检查它是否已经被解决了。
...
*`throwTacticEx` vs `throwError`*：`throwTacticEx` 接受一个 {lean}`MVarId` 参数，抛出的错误信息会包含该目标的完整上下文（包括所有假设和目标类型）。这对用户来说非常有价值——他们不需要自己去翻 Infoview 就能看到出错时的状态。`throwError` 则是通用的错误抛出函数，不附带目标上下文，适合用于和特定目标无关的错误（比如参数解析失败）。

*{lean}`logWarning`*：发出警告但不中断执行。适合用于"这个情况可能有问题但不确定"的场景。
...
如果你在证明过程中只有一个目标、且没有经过 {kw}`intro` 或 {kw}`cases` 之类改变上下文的操作，这段代码可能"碰巧"能工作。但一旦有多个目标或上下文被修改过，{lean}`getLCtx` 拿到的就不是当前目标的上下文，而是"默认"上下文（通常是最外层的），导致找不到该有的假设，或者更糟——找到错误的假设。
...
{lean}`isDefEq` 不是一个纯函数——它会修改元变量的状态。当你调用 `isDefEq a b`，如果 `a` 或 `b` 中包含未实例化的元变量，{lean}`isDefEq` 可能会把它们实例化来使等式成立。
...
1. *不要把 {lean}`isDefEq` 当作"无害的检查"来用*。如果你只是想"试试看两个类型是否相同"但不想产生任何副作用，你需要用 `withoutModifyingState` 包裹：`withoutModifyingState (isDefEq a b)`。

2. *遍历假设时注意顺序*。如果你先用 {lean}`isDefEq` 匹配了假设 A，虽然最终没用它（比如后来发现假设 B 更好），但 A 的匹配过程可能已经实例化了某些元变量，影响后续的匹配。这是 3.5 节中需要 `g.isAssigned` 检查的根本原因。
...
问题在哪？如果调用 `bad_split` 时目标列表是 `[goal, g2, g3]`，{lean}`setGoals` 只设置了 `[mvarP, mvarQ]`，那 `g2` 和 `g3` 就从目标列表中消失了。用户看不到它们，也无法用后续 tactic 处理它们——但它们还在那里，未被证明，最终会导致"unsolved goals"错误。
...
提示：
- 目标 `P → Q` 在 {lean}`Expr` 层面是 `Expr.forallE name P Q info`
- 用 `goal.intro name` 来引入——它会自动创建新的局部假设并返回新目标
- `goal.intro` 返回的是新目标的 {lean}`MVarId`，你需要 `replaceMainGoal [newGoal]` 来更新
...
下面的 tactic 试图对所有目标执行 {lean}`rfl`，但有两个 bug。找出并修复它们。
...
提示：bug 1 与 {lean}`setGoals` 有关（参考 3.7 节），bug 2 与 `Eq` 的参数个数有关（`@Eq α a b` 有三个参数，不是两个）。
...
- 不要忘记用 `(← getGoals).erase goal` 保留其他目标，或者用 {lean}`replaceMainGoal`
...
- {lean}`getMainGoal` + `goal.withContext`：获取目标并切换上下文
- `goal.getType`：读取目标命题
- `goal.assign proof`：用证明项关闭目标
- `mkFreshExprMVar T`：创建类型 T 的新 hole（子目标）
- {lean}`setGoals`：更新目标列表
- `isDefEq a b`：判断定义等价（核心匹配操作）
- `throwTacticEx`：带目标上下文的错误信息
- {lean}`evalTactic`：在 tactic 中调用其他 tactic
- `withoutModifyingState`：不产生副作用地执行操作

*核心原则*：tactic = 读目标 + 分析结构 + 构造证明项 + 更新目标列表。

本章你学到了编写 tactic 的完整流程：从最简单的 {kw}`assumption` 到拆分目标的 `split_and`，从组合已有 tactic 到对多目标批量操作，再到错误处理和常见陷阱。每个 tactic 都遵循同一个模式——读取目标、分析 {lean}`Expr` 结构、构造证明项、更新目标列表。
