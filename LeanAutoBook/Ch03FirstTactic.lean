...
2. *读取状态*：从 `TacticM` monad 中获取当前目标列表。最常用的是 `getMainGoal`（取第一个目标）和 `getGoals`（取全部目标）。每个目标是一个 `MVarId`——元变量的唯一标识符。

3. *分析目标*：通过 `goal.getType` 拿到目标的类型（一个 `Expr`），然后用第二章学到的模式匹配技术分析它的结构。比如判断它是否形如 `P ∧ Q`，或者是否是一个等式 `a = b`。

4. *构造证明项并更新状态*：这是最关键的一步。你需要构造一个 `Expr` 来"回答"当前目标。如果证明项中还有未填充的部分（hole），那些 hole 就变成新的子目标。最后调用 `goal.assign proof` 关闭当前目标，调用 `setGoals` 设置新的目标列表。
...
*`let goal ← getMainGoal`*：从当前证明状态中取出第一个未解决的目标。`getMainGoal` 返回 `MVarId`，它是元变量的标识符。为什么叫"main goal"？因为 Lean 的目标列表是有序的，第一个目标就是用户在 Infoview 中看到的那个。
...
*`if ← isDefEq decl.type target then`*：这是核心判断。`isDefEq` 检查两个表达式是否 definitionally equal——不只是语法层面的相等，而是考虑 beta 规约（`(fun x => x) a ≡ a`）、delta 规约（展开定义）、iota 规约（结构体投影和匹配）等所有计算规则之后的相等性。如果假设的类型和目标匹配，我们就找到了证明。
...
> - *需要完全控制整个目标列表*时（比如重新排序、批量过滤），才用 `setGoals`。
>
> 经验法则：如果你的 tactic 只处理一个目标并产出若干子目标，`replaceMainGoal` 更简洁也更安全。

# 3.4 调用已有 tactic：组合式证明
...
1. *不要把 `isDefEq` 当作"无害的检查"来用*。如果你只是想"试试看两个类型是否相同"但不想产生任何副作用，你需要用 `withoutModifyingState` 包裹：`withoutModifyingState (isDefEq a b)`。

2. *遍历假设时注意顺序*。如果你先用 `isDefEq` 匹配了假设 A，虽然最终没用它（比如后来发现假设 B 更好），但 A 的匹配过程可能已经实例化了某些元变量，影响后续的匹配。这是 3.5 节中需要 `g.isAssigned` 检查的根本原因。
