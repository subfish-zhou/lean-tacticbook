...
遇到 `Iff`，它要求左边是一条等式，再取这条等式的左边。遇到裸结论，它会遍历局部上下文，跳过实现细节，取第一条左边含有所推断自由变量的等式。这里两条来源路径的范围并不相同：`Iff` 路径可以继续携带复合的共同左端；局部上下文路径受 `x.isFVar` 限制，只支持共同左端本身就是自由变量。当前辅助函数只按原方向取第一个合格假设：不反转等式，不检查另一边是否为零，不比较多个候选，也不展开定义。
...
`liftMetaTactic` 把一个活动的 `MVarId` 交给 MetaM 函数，接过函数返回的替代目标，再负责接好队列；它定义在 `Lean/Elab/Tactic/Basic.lean` 的目标队列操作旁边。这里 `goal.withContext` 进入该目标的局部上下文。搜索从较新的声明走向较旧的声明，忽略实现细节，并用 `isDefEq` 比较每个声明的类型与目标类型。
...
“接收一个 `MVarId`，返回取代它的一组 `MVarId`”正是 *`liftMetaTactic`* 接受的形状。`my_assumption` 关闭传入的主目标，交回空列表；apply 一类的 Meta 辅助函数若已按所需顺序返回目标，也可以直接接上。`liftMetaTactic` 会进入主目标并替换队首，却不会替任意 Meta 计算补上生产级事务语义。

已有成熟证明术能完成操作时，译补器只需检查状态，选择或构造调用，再交给 *`evalTactic`* 运行。`poly_roots` 和 `my_step` 都走这里。被委托的证明术无论创建目标、关闭目标、失败，还是报告诊断，效果都会直接留在当前证明状态里。
...
`MVarId.apply` 仍把旧元变量赋成 `And.intro ?left ?right`，可是证明术队列里仍然只有那个已经赋值、如今陈旧的队首。下一个圆点向 `getMainGoal` 要工作；它跳过已赋值的条目，一路走到队列末尾，于是报告 `No goals to be solved`。未解的 `?left` 和 `?right` 确实存在于元变量上下文中，但 TacticM 不会自动发现并把它们当成活动目标。
...
1. *改进 `poly_roots` 的诊断。* 修改 `sourcePolynomial?`：对每个候选等式的右边使用 `rhs.nat? == some 0`，只接受这种结构上的数值字面量零；这项检查不承诺识别所有定义等于零的表达式。若没有候选通过，公开译补器就在整个调用处报告来源错误。把 `hBad : x + 1 = 1` 放在 `hGood : x^2 - 5*x + 6 = 0` 之前，并以 `x = 2 ∨ x = 3` 为目标：未检查右边的实现会误选 `x + 1` 并在委托证明中失败，正确实现才会跳过它并接受 `hGood`。再加入只有非零右边的反例，确认执行不会走到 `ring`。这改变的是识别，不是数学，也不要求核心记住某个声明身份。
