...
这一章先走另一条路。后面四章的常用接口，我们按眼前任务借一点，用一点。每次借用都说明它此刻能保证什么，不能保证什么；内部结构等读者亲手遇到问题以后再偿还。这样不是省略理论，而是先让理论有债主。
...
当前 helper 的契约比名称窄。它把析取叶子当作等式，要求所有左边在 Expr 结构上相同，却不把公共左式验证成自由变量；处理双条件时，它直接拿左边等式的左侧当来源式，没有核对等式右侧一定是零。处理局部上下文时，它只接受公共左式恰为自由变量的情况，遇到多个候选会静默取先碰到的一个。`rootEqualities?` 虽能展平任意形状的 Or 树，后面的宏核心却重建右结合析取，所以左结合目标仍可能在最后对接时失败；单个根也没有被当前脚本完整处理。最后，宏核心只验证候选根给出的首一因式乘积；非首一情形还要另外处理首项系数并证明它非零。这些都不是雅印上的小瑕疵，而是下一版接口必须先写清的输入契约。
